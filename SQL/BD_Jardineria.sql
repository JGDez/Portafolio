...
CREATE TABLE oficina (
    codigo_oficina VARCHAR(10) NOT NULL PRIMARY KEY COMMENT 'Primary key',
...
CREATE TABLE gama_producto (
    gama VARCHAR(50) NOT NULL PRIMARY KEY COMMENT 'Primary key',
...
CREATE TABLE pago (
    ...
    id_transaccion VARCHAR(50) NOT NULL PRIMARY KEY COMMENT 'Primary key',
...
Código con datos copiado de <https://gist.github.com/josejuansanchez/c408725e848afd64dd9a20ab37fba8c9>
*/
...
SELECT codigo_oficina AS 'Cód. Oficina', ciudad AS 'Ciudad' FROM oficina;
...
SELECT nombre AS 'Nombre', CONCAT(apellido1, ' ', COALESCE(apellido2)) AS "Apellidos", email AS 'Correo-e' FROM empleado WHERE codigo_jefe = 7 ORDER BY apellido1, apellido2, nombre;

/*
 4. Devuelve el nombre del puesto, nombre, apellidos y email del jefe de la empresa.
*/

--Opción 1: Buscar al jefe por el nombre del puesto
SELECT puesto AS 'Puesto', nombre AS 'Nombre', CONCAT(apellido1, ' ', COALESCE(apellido2)) AS "Apellidos", email AS 'Correo-e' FROM empleado WHERE LCASE(puesto) = 'director general';
--Opción 2: Buscar al jefe sabiendo que no tiene jefe (NULL)
SELECT puesto AS 'Puesto', nombre AS 'Nombre', CONCAT(apellido1, ' ', COALESCE(apellido2)) AS "Apellidos", email AS 'Correo-e' FROM empleado WHERE codigo_jefe IS NULL;

/*
 5. Devuelve un listado con el nombre, apellidos y puesto de aquellos empleados que no sean representantes de ventas.
*/

SELECT nombre AS 'Nombre', CONCAT(apellido1, ' ', COALESCE(apellido2)) AS 'Apellidos', puesto AS 'Puesto' FROM empleado WHERE LCASE(puesto) = 'representante ventas' ORDER BY nombre, apellido1, apellido2;

/*
 6. Devuelve un listado con el nombre de los todos los clientes españoles.
*/

SELECT nombre_cliente AS 'Nombre' FROM cliente WHERE LCASE(pais) = 'spain' ORDER BY nombre_cliente;

/*
 7. Devuelve un listado con los distintos estados por los que puede pasar un pedido.
*/

SELECT estado FROM pedido GROUP BY estado ORDER BY estado;
...
--Utilizando la función YEAR de MYSQL
SELECT cli.codigo_cliente AS 'Código Cliente' FROM cliente AS cli JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente) WHERE YEAR(p.fecha_pago) = 2008 GROUP BY cli.codigo_cliente;

--Utilizando la función DATE_FORMAT de MYSQL
SELECT cli.codigo_cliente AS 'Código Cliente' FROM cliente AS cli JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente) WHERE DATE_FORMAT(p.fecha_pago,'%Y') = 2008 GROUP BY cli.codigo_cliente;
--Sin ninguna de las funciones de fecha anteriores
SELECT cli.codigo_cliente AS 'Código Cliente' FROM cliente AS cli JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente) WHERE p.fecha_pago BETWEEN '2008-01-01' AND '2008-12-31' GROUP BY cli.codigo_cliente;

/*
 9. Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos que no han sido entregados a tiempo.
*/

SELECT codigo_pedido AS 'Cód. Pedido', codigo_cliente AS 'Cód. Cliente', fecha_esperada AS 'Fecha esperada', fecha_entrega AS 'Fecha entrega' FROM pedido WHERE fecha_esperada < fecha_entrega;
-- Otra forma de saber si no ha sido entregado a tiempo, calculando los días de demora.
SELECT codigo_pedido AS 'Cód. Pedido', codigo_cliente AS 'Cód. Cliente', fecha_esperada AS 'Fecha esperada', fecha_entrega AS 'Fecha entrega' FROM pedido WHERE fecha_entrega - fecha_esperada > 0;
...
-- Utilizando la función ADDDATE de MySQL
SELECT codigo_pedido AS 'Cód. Pedido', codigo_cliente AS 'Cód. Cliente', fecha_esperada AS 'Fecha esperada', fecha_entrega AS 'Fecha entrega' FROM pedido WHERE fecha_entrega <= ADDDATE(fecha_esperada, INTERVAL -2 DAY);
-- Utiliznado la función DATEDIFF de MYSQL
SELECT codigo_pedido AS 'Cód. Pedido', codigo_cliente AS 'Cód. Cliente', fecha_esperada AS 'Fecha esperada', fecha_entrega AS 'Fecha entrega' FROM pedido WHERE DATEDIFF(fecha_esperada, fecha_entrega) >= 2;
-- Utilizando el operador resta (-).
SELECT codigo_pedido AS 'Cód. Pedido', codigo_cliente AS 'Cód. Cliente', fecha_esperada AS 'Fecha esperada', fecha_entrega AS 'Fecha entrega' FROM pedido WHERE fecha_esperada - fecha_entrega >= 2;

/*
 11. Devuelve un listado de todos los pedidos que fueron rechazados en 2009.
*/

SELECT * FROM pedido WHERE LOWER(estado) = 'rechazado' AND YEAR(fecha_pedido) = 2009;

/*
 12. Devuelve un listado de todos los pedidos que han sido entregados en el mes de enero de cualquier año.
*/

SELECT * FROM pedido WHERE MONTH(fecha_entrega) = 1 ORDER BY fecha_entrega;

/*
 13. Devuelve un listado con todos los pagos que se realizaron en el año 2008 mediante Paypal. Ordene el resultado de mayor a menor.
*/

SELECT * FROM pago WHERE LOWER(forma_pago) = "paypal" AND YEAR(fecha_pago) = 2008;

/*
 14. Devuelve un listado con todas las formas de pago que aparecen en la tabla pago. Tenga en cuenta que no deben aparecer formas de pago repetidas.
*/

SELECT forma_pago AS 'Forma de pago' FROM pago GROUP BY forma_pago;
...
SELECT cli.nombre_cliente AS 'Nombre cliente', CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Representante de ventas' FROM cliente AS cli JOIN empleado AS emp  ON (emp.codigo_empleado = cli.codigo_empleado_rep_ventas);

/*
 2. Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
*/

SELECT DISTINCT cli.nombre_cliente AS 'Nombre cliente', CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Representante de ventas'
 FROM cliente AS cli JOIN pago AS p ON(cli.codigo_cliente = p.codigo_cliente) 
 JOIN empleado AS emp ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado) 
...
/* Nota: Única forma sin utilizar las composiciones externas fue
 usando una subconsulta para filtrar los clientes que no están en la 
 tabla pago. */
SELECT DISTINCT cli.nombre_cliente AS 'Nombre cliente', CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Representante de ventas' FROM cliente AS cli JOIN empleado AS emp ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado) WHERE cli.codigo_cliente NOT IN (SELECT pago.codigo_cliente FROM pago) ORDER BY cli.nombre_cliente;

/*
 4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
*/

SELECT DISTINCT cli.nombre_cliente AS 'Nombre cliente', CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Representante de ventas', ofi.ciudad AS 'Ciudad oficina'
 FROM cliente AS cli JOIN pago AS p ON(cli.codigo_cliente = p.codigo_cliente) 
 JOIN empleado AS emp ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado) 
 JOIN oficina AS ofi ON (emp.codigo_oficina = ofi.codigo_oficina) 
 ORDER BY cli.nombre_cliente;
...
SELECT DISTINCT cli.nombre_cliente AS 'Nombre cliente', CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Representante de ventas', ofi.ciudad AS 'Ciudad oficina'
 FROM cliente AS cli JOIN empleado AS emp ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado) 
 JOIN oficina AS ofi ON (emp.codigo_oficina = ofi.codigo_oficina) 
 WHERE cli.codigo_cliente NOT IN (
    SELECT pago.codigo_cliente FROM pago apellido1
 ) ORDER BY cli.nombre_cliente;
...
SELECT CONCAT(ofi.linea_direccion1, ' ', COALESCE(ofi.linea_direccion2), ', ', ofi.codigo_postal, ' ', ofi.ciudad, ', ', ofi.region, ', ', ofi.pais) AS 'Dirección oficina' FROM oficina AS ofi
 JOIN empleado AS emp ON (ofi.codigo_oficina = emp.codigo_oficina)
 JOIN cliente AS cli ON (emp.codigo_empleado = cli.codigo_empleado_rep_ventas)
 WHERE LOWER(cli.ciudad) = 'fuenlabrada'
...
SELECT cli.nombre_cliente AS 'Cliente', CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Representante', ofi.ciudad AS 'Ciudad' FROM cliente AS cli
 JOIN empleado AS emp ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado)
 JOIN oficina AS ofi ON (emp.codigo_oficina = ofi.codigo_oficina)
 ORDER BY cli.nombre_cliente
;
...
SELECT CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Empleado', CONCAT(jefe.nombre, ' ', jefe.apellido1, ' ', COALESCE(jefe.apellido2)) AS 'Jefe'
 FROM empleado AS emp
 JOIN empleado AS jefe ON (emp.codigo_jefe = jefe.codigo_empleado)
...
SELECT 
    CONCAT(emp.nombre, ' ', emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Empleado',
    CONCAT(jefe.nombre, ' ', jefe.apellido1, ' ', COALESCE(jefe.apellido2)) AS 'Jefe',
    CONCAT(jsup.nombre, ' ', jsup.apellido1, ' ', COALESCE(jsup.apellido2)) AS 'Superior'
 FROM empleado AS emp
 JOIN empleado AS jefe ON (emp.codigo_jefe = jefe.codigo_empleado)
 JOIN empleado AS jsup ON (jefe.codigo_jefe = jsup.codigo_empleado)
 ORDER BY emp.codigo_empleado
;
...
SELECT DISTINCT cli.nombre_cliente AS 'Cliente' FROM cliente AS cli
 JOIN pedido AS p ON (p.codigo_cliente = cli.codigo_cliente)
...
SELECT DISTINCT cli.nombre_cliente AS 'Cliente', gp.gama AS 'Gama' FROM cliente AS cli
 JOIN pedido AS p ON (p.codigo_cliente = cli.codigo_cliente)
 JOIN detalle_pedido AS dp ON (dp.codigo_pedido = p.codigo_pedido)
 JOIN producto AS pro ON (pro.codigo_producto = dp.codigo_producto)
 JOIN gama_producto AS gp ON (gp.gama = pro.gama)
 ORDER BY cli.nombre_cliente
;
...
SELECT cli.nombre_cliente AS 'Cliente' FROM cliente AS cli
 LEFT JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente)
...
SELECT cli.nombre_cliente AS 'Cliente' FROM cliente AS cli
 LEFT JOIN pedido AS p ON (p.codigo_cliente = cli.codigo_cliente)
 WHERE p.codigo_cliente IS NULL
 GROUP BY cli.codigo_cliente
 ORDER BY cli.nombre_cliente
;
...
SELECT DISTINCT cli.nombre_cliente AS 'Cliente' FROM cliente AS cli
 LEFT JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente)
 LEFT JOIN pedido AS ped ON (ped.codigo_cliente = cli.codigo_cliente)
 WHERE p.codigo_cliente IS NULL || ped.codigo_cliente IS NULL
 ORDER BY cli.nombre_cliente
;
...
SELECT emp.nombre AS 'Nombre', CONCAT(emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Apellidos'
 FROM empleado AS emp
 LEFT JOIN oficina AS ofi ON (ofi.codigo_oficina = emp.codigo_oficina)
 WHERE ofi.codigo_oficina IS NULL
 ORDER BY 'Nombre', 'Apellidos'
;
-- En esta BD tod@s l@s trabajador@s tienen una oficina asignada.

/*
 5. Devuelve un listado que muestre solamente los empleados que no tienen un cliente asociado.
*/

SELECT emp.nombre AS 'Nombre', CONCAT(emp.apellido1, ' ', COALESCE(emp.apellido2))
 FROM empleado AS emp
 LEFT JOIN cliente AS cli ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado)
...
SELECT emp.nombre AS 'Nombre', CONCAT(emp.apellido1, ' ', COALESCE(emp.apellido2)), ofi.codigo_oficina AS 'Cód oficina'
 FROM empleado AS emp
 LEFT JOIN cliente AS cli ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado)
 LEFT JOIN oficina AS ofi ON (ofi.codigo_oficina = emp.codigo_oficina)
 WHERE cli.codigo_empleado_rep_ventas IS NULL
...
SELECT emp.nombre AS 'Nombre', CONCAT(emp.apellido1, ' ', COALESCE(emp.apellido2)), ofi.codigo_oficina AS 'Cód oficina'
 ...
 LEFT JOIN cliente AS cli ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado)
 LEFT JOIN oficina AS ofi ON (ofi.codigo_oficina = emp.codigo_oficina)
 WHERE cli.codigo_empleado_rep_ventas IS NULL || ofi.codigo_oficina IS NULL
;
-- Sólo aparecen l@s que no tienen cliente asignado porque tod@s tienen oficina asignada

/*
 8. Devuelve un listado de los productos que nunca han aparecido en un pedido.
*/

SELECT DISTINCT pro.nombre AS 'Producto' FROM producto AS pro
 LEFT JOIN detalle_pedido AS dp ON (dp.codigo_producto = pro.codigo_producto)
 LEFT JOIN pedido AS ped ON (ped.codigo_pedido = dp.codigo_pedido)
...
SELECT DISTINCT pro.nombre AS 'Producto', pro.descripcion AS 'Descripción', gp.imagen AS 'Imagen gama' FROM producto AS pro
 LEFT JOIN detalle_pedido AS dp ON (dp.codigo_producto = pro.codigo_producto)
 LEFT JOIN pedido AS ped ON (ped.codigo_pedido = dp.codigo_pedido)
 LEFT JOIN gama_producto AS gp ON (gp.gama = pro.gama)
 WHERE ped.codigo_pedido IS NULL
 ORDER BY pro.nombre
;
/* En la BD no hay foto de producto, sólo de la gama aunque todas están como NULL.
 Las descripciones también están como NULL */
...
SELECT ofi.codigo_oficina AS 'Cód. oficina' FROM oficina AS ofi
 LEFT JOIN empleado AS emp ON (emp.codigo_oficina = ofi.codigo_oficina)
 LEFT JOIN cliente AS cli ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado)
 LEFT JOIN pedido AS ped ON (ped.codigo_cliente = cli.codigo_cliente)
 LEFT JOIN detalle_pedido AS dp ON (dp.codigo_pedido = ped.codigo_pedido)
 LEFT JOIN producto AS pro ON (pro.codigo_producto = dp.codigo_producto)
 LEFT JOIN gama_producto AS gp ON (gp.gama = pro.gama)
 WHERE LOWER(gp.gama) NOT IN('frutales')
 GROUP BY ofi.codigo_oficina
;
...
SELECT cli.nombre_cliente AS 'Cliente' FROM cliente AS cli
 LEFT JOIN pedido AS ped ON (ped.codigo_cliente = cli.codigo_cliente)
 LEFT JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente)
 WHERE ped.codigo_cliente IS NOT NULL AND p.id_transaccion IS NULL
 GROUP BY cli.codigo_cliente
;
...
SELECT emp.*, jefe.nombre AS 'Nombre Jefe', CONCAT(jefe.apellido1, ' ', COALESCE(jefe.apellido2)) AS 'Apellidos jefe' FROM empleado AS emp
 LEFT JOIN empleado AS jefe ON (jefe.codigo_empleado = emp.codigo_jefe)
 LEFT JOIN cliente AS cli ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado)
 WHERE cli.codigo_empleado_rep_ventas IS NULL
 ORDER BY emp.nombre, emp.apellido1, emp.apellido2 ASC
;
...
SELECT COUNT(codigo_empleado) AS 'Nº Empleados' FROM empleado;

/*
 2. ¿Cuántos clientes tiene cada país?
*/

SELECT pais AS 'País', COUNT(codigo_cliente) AS 'Nº Clientes' FROM cliente GROUP BY pais;

/*
 3. ¿Cuál fue el pago medio en 2009?
*/

SELECT YEAR(fecha_pago) AS Año, AVG(total) AS 'Media pago' FROM pago GROUP BY Año ORDER BY Año;

/*
 4. ¿Cuántos pedidos hay en cada estado? Ordena el resultado de forma descendente por el número de pedidos.
*/

SELECT estado AS 'Estado', COUNT(codigo_pedido) AS 'Nº Pedidos' FROM pedido GROUP BY estado ORDER BY COUNT(codigo_pedido) DESC;

/*
 5. Calcula el precio de venta del producto más caro y más barato en una misma consulta.
*/

SELECT MAX(precio_venta) AS 'Mas caro', MIN(precio_venta) AS 'Mas barato' FROM producto;

/*
 6. Calcula el número de clientes que tiene la empresa.
*/

SELECT COUNT(codigo_cliente) AS 'Nº clientes' FROM cliente;

/*
 7. ¿Cuántos clientes existen con domicilio en la ciudad de Madrid?
*/

SELECT COUNT(codigo_cliente) AS 'Clientes en Madrid' FROM cliente WHERE LOWER(ciudad) = 'madrid';

/*
 8. ¿Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M?
*/

SELECT ciudad AS 'Ciudad', COUNT(codigo_cliente) AS 'Nº Clientes' FROM cliente WHERE REGEXP_LIKE(ciudad, '^[mM].*') GROUP BY ciudad;

/*
 9. Devuelve el nombre de los representantes de ventas y el número de clientes al que atiende cada uno.
*/

SELECT emp.nombre AS 'Nombre', CONCAT(emp.apellido1, ' ', COALESCE(emp.apellido2)) AS 'Apellidos', COUNT(cli.codigo_cliente) AS 'Nº clientes'
 FROM empleado AS emp 
 JOIN cliente AS cli ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado) 
...
SELECT COUNT(codigo_cliente) AS 'Nº clientes sin prepresentante' FROM cliente WHERE codigo_empleado_rep_ventas IS NULL;
-- En esta BD todos los clientes tienen representante de ventas asignado.

/*
 11. Calcula la fecha del primer y último pago realizado por cada uno de los clientes. El listado deberá mostrar el nombre y los apellidos de cada cliente.
*/

SELECT cli.nombre_cliente AS 'Cliente', cli.nombre_contacto AS 'Nombre', cli.apellido_contacto AS 'Apellido', MIN(p.fecha_pago) AS 'Primer pago', MAX(p.fecha_pago) AS 'Último pago'
 FROM cliente AS cli 
 JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente) 
 GROUP BY cli.codigo_cliente
;
...
SELECT codigo_pedido AS 'Pedido', COUNT(codigo_producto) AS 'Productos diferentes' FROM detalle_pedido
...
-- Entendiendo que cantidad total se refiere número de productos y no al precio total.
SELECT codigo_pedido AS 'Pedido', SUM(cantidad) AS 'Total productos' FROM detalle_pedido
 GROUP BY codigo_pedido
 ORDER BY codigo_pedido
;

-- Entendiendo que cantidad total se refiere al precio total y no al número de productos.
SELECT codigo_pedido AS 'Pedido', sum(cantidad * precio_unidad) AS 'Precio total pedido' FROM detalle_pedido
 GROUP BY codigo_pedido
 ORDER BY codigo_pedido
;
...
SELECT pro.nombre AS 'Producto', SUM(dp.cantidad) AS 'Total unidades' FROM producto AS pro
 JOIN detalle_pedido AS dp ON (dp.codigo_producto = pro.codigo_producto)
...
SELECT 
    SUM(cantidad * precio_unidad) AS 'Base Imponible', 
    ROUND(21 * (SUM(cantidad * precio_unidad) / 100), 2) AS 'IVA', 
    ROUND(SUM(cantidad * precio_unidad) + (21 * (SUM(cantidad * precio_unidad) / 100)),2) AS 'Total'
 FROM detalle_pedido
;
...
SELECT 
    codigo_producto AS 'Cód. Producto',
    SUM(cantidad * precio_unidad) AS 'Base Imponible', 
    ROUND(21 * (SUM(cantidad * precio_unidad) / 100), 2) AS 'IVA', 
    ROUND(SUM(cantidad * precio_unidad) + (21 * (SUM(cantidad * precio_unidad) / 100)),2) AS 'Total'
 FROM detalle_pedido
...
SELECT 
    ...
    ROUND(SUM(cantidad * precio_unidad) + (21 * (SUM(cantidad * precio_unidad) / 100)),2) AS 'Total'
 FROM detalle_pedido
 WHERE REGEXP_LIKE(codigo_producto, '^OR.*')
 GROUP BY codigo_producto
 ORDER BY codigo_producto
;
...
SELECT 
    ...
    COUNT(dp.codigo_producto),
    SUM(dp.cantidad * dp.precio_unidad) AS 'Base Imponible', 
    ROUND(21 * (SUM(dp.cantidad * dp.precio_unidad) / 100), 2) AS 'IVA', 
    ROUND(SUM(dp.cantidad * dp.precio_unidad) + (21 * (SUM(dp.cantidad * dp.precio_unidad) / 100)),2) AS 'Total'
...
*/
    ...
    SELECT YEAR(fecha_pago) AS Año, SUM(total) AS 'Total' FROM pago GROUP BY Año
...
SELECT nombre AS 'Producto' FROM producto
 WHERE codigo_producto = (
    SELECT codigo_producto FROM detalle_pedido GROUP BY codigo_producto ORDER BY MAX(cantidad) DESC LIMIT 1
 );
...
SELECT cli.nombre_cliente AS 'Cliente', cli.limite_credito AS 'Límite', sum_p.sum_total AS 'Pagos' FROM cliente AS cli, (
    SELECT codigo_cliente AS cod_cli, SUM(total) AS sum_total FROM pago GROUP BY codigo_cliente
) AS sum_p
 WHERE cli.codigo_cliente = sum_p.cod_cli AND cli.limite_credito > sum_p.sum_total
 ORDER BY cli.nombre_cliente
;
...
-- No sería correcto mostrar un solo producto porque hay varios que cumplen ser los que tienen más stock.
...
-- No sería correcto mostrar un solo producto porque puede haber varios que cumplen ser los que tienen menos stock.

/*
 7. Devuelve el nombre, los apellidos y el email de los empleados que están a cargo de *Alberto Soria*.
*/

SELECT nombre AS 'Nombre', CONCAT(apellido1, ' ', COALESCE(apellido2)) AS 'Apellidos', email FROM empleado
 WHERE codigo_jefe = (
    SELECT codigo_empleado FROM empleado WHERE CONCAT(nombre, ' ', apellido1) = 'Alberto Soria'
 );

 --Con patrones se podría hacer
SELECT nombre AS 'Nombre', CONCAT(apellido1, ' ', COALESCE(apellido2)) AS 'Apellidos', email FROM empleado
 WHERE codigo_jefe = (
    SELECT codigo_empleado FROM empleado WHERE LOWER(CONCAT(nombre, ' ', apellido1)) LIKE LOWER('%Alberto Soria%')
 ); 
 --Se pasa todo a minúsculas para evitar diferencias al escribir.
...
SELECT nombre  AS 'Nombre', apellido1 AS 'Apellido' FROM empleado
...
SELECT nombre  AS 'Nombre', CONCAT(apellido1, ' ', COALESCE(apellido2)) AS 'Apellidos', puesto, extension FROM empleado
 WHERE codigo_empleado NOT IN (
    SELECT codigo_empleado_rep_ventas FROM cliente
...
SELECT emp.codigo_oficina FROM empleado AS emp
 WHERE emp.codigo_empleado NOT IN (
  SELECT cli.codigo_cliente FROM cliente AS cli, pedido AS ped, detalle_pedido AS dp, producto AS pro
    WHERE cli.codigo_cliente = ped.codigo_cliente 
      AND ped.codigo_pedido = dp.codigo_pedido 
      AND dp.codigo_producto = pro.codigo_producto
      AND LOWER(pro.gama) = 'frutales'
    GROUP BY cli.codigo_cliente
 )
 GROUP BY emp.codigo_oficina
 ORDER BY emp.codigo_oficina
;
...
SELECT cli.nombre_cliente FROM cliente AS cli
    WHERE cli.codigo_cliente IN (
        SELECT ped.codigo_cliente FROM pedido AS ped
...
SELECT cli.nombre_cliente FROM cliente AS cli
    WHERE NOT EXISTS (
        ...
        WHERE p.codigo_cliente = cli.codigo_cliente
...
SELECT pro.nombre FROM producto AS pro
    WHERE NOT EXISTS (
        SELECT * FROM detalle_pedido AS dp
        WHERE dp.codigo_producto = pro.codigo_producto
        GROUP BY dp.codigo_producto
    )
    GROUP BY pro.codigo_producto
    ORDER BY pro.codigo_producto
;
...
SELECT cli.nombre_cliente AS 'Cliente', COUNT(ped.codigo_cliente) AS 'Nº pedidos' FROM cliente AS cli
    JOIN pedido AS ped ON (ped.codigo_cliente = cli.codigo_cliente)
    GROUP BY cli.nombre_cliente
    ORDER BY cli.nombre_cliente
;
...
SELECT cli.nombre_cliente AS 'Cliente', SUM(p.total) FROM cliente AS cli
    JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente)
    GROUP BY cli.codigo_cliente
    ORDER BY cli.nombre_cliente
;
...
SELECT cli.nombre_cliente FROM cliente AS cli
    JOIN pedido AS ped ON (ped.codigo_cliente = cli.codigo_cliente)
    WHERE YEAR(ped.fecha_pedido) = 2008
    GROUP BY cli.codigo_cliente
...
SELECT cli.nombre_cliente AS 'Cliente', emp.nombre AS 'Nombre', emp.apellido1 AS 'Apellido', ofi.telefono AS 'Tel. Oficina', p.codigo_cliente FROM cliente AS cli
    JOIN empleado AS emp ON (emp.codigo_empleado = cli.codigo_empleado_rep_ventas)
    JOIN oficina AS ofi ON (ofi.codigo_oficina = emp.codigo_oficina)
    LEFT JOIN pago AS p ON (p.codigo_cliente = cli.codigo_cliente)
    WHERE p.codigo_cliente IS NULL
    GROUP BY cli.codigo_cliente
    ORDER BY cli.nombre_cliente
...
SELECT cli.nombre_cliente AS 'Cliente', emp.nombre AS 'Nombre', emp.apellido1 AS 'Apellido', ofi.ciudad AS 'Ciudad' FROM cliente AS cli
    JOIN empleado AS emp ON (emp.codigo_empleado = cli.codigo_empleado_rep_ventas)
    JOIN oficina AS ofi ON (ofi.codigo_oficina = emp.codigo_oficina)
    ORDER BY cli.nombre_cliente
;
...
SELECT emp.nombre AS Nombre, CONCAT(emp.apellido1, ' ', COALESCE(emp.apellido2)) AS Apellidos, emp.puesto As 'Puesto', ofi.telefono AS 'Tel. Oficina' FROM empleado AS emp
    JOIN oficina AS ofi ON (ofi.codigo_oficina = emp.codigo_oficina)
    LEFT JOIN cliente AS cli ON (cli.codigo_empleado_rep_ventas = emp.codigo_empleado)
    WHERE cli.codigo_empleado_rep_ventas IS NULL
...
SELECT ofi.ciudad AS 'Ciudad', COUNT(emp.codigo_empleado) AS 'Nº de empleados' FROM oficina AS ofi
    JOIN empleado AS emp ON (emp.codigo_oficina = ofi.codigo_oficina)
    GROUP BY ofi.ciudad
;
